  ...
  while (mmax < nn)
    {
      ...
      const double delta = 2. * M_PI * isign / mmax;

      const double wtemp = sin(0.5 * delta);
      const double alpha = 2. * wtemp*wtemp;
      ...
      for (int m=1; m<mmax; m+=2)
	{
	  for (int i=m; i<=nn; i+=istep)
	    {
	      const int k = i + mmax;
	      const double tempr = wr * data[k-1] - wi * data[k];
	      const double tempi = wr * data[k]   + wi * data[k-1];
...
void fft(VecDoz& data, const int isign)
{
  ...
  for (int i=0; i<n; ++i)
    {
      const double real = tmp[2*i];
      const double imag = tmp[2*i+1];
